        ...
        <div className='flex flex-col gap-7 justify-around md:grid grid-cols-2 md:gap-0 px-6 mt-9'>
            ...
            <div className='flex flex-col items-start gap-5'>
                <div className='flex flex-col gap-3 w-full'>
                    ...
                    <textarea name="message" rows={5} placeholder='Type your message' id="message" className='border border-[#3C5065] p-2 rounded-lg' />
                </div>
                <div className='flex items-center mt-4'>
                    <input type="checkbox" id="terms" className='mr-2' onChange={(e) => setAcceptTerms(e.target.checked)} />
                    <label htmlFor="terms">I accept the terms</label>
                </div>
                <button className={`bg-[#3C5065] text-white rounded-xl p-2 px-6 ${!acceptTerms && 'opacity-70'}`} disabled={!acceptTerms}>Submit</button>
            </div>
        </div>
    </div>
  )
